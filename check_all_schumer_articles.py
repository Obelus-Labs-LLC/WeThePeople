...
db = SessionLocal()
try:
    print("Checking all Schumer claim URLs for bill numbers...\n")
    
    schumer_claims = db.query(Claim).filter(Claim.person_id == 'chuck_schumer').all()
    
    urls_checked = set()
    bills_found = 0
    
    for claim in schumer_claims:
        if claim.claim_source_url in urls_checked:
            continue
        urls_checked.add(claim.claim_source_url)
        
        print(f"Checking: {claim.claim_source_url.split('/')[-1][:50]}...")
