...
type DomeGalleryProps = {
  ...
  grayscale?: boolean;
  onItemClick?: (index: number, item: ItemDef) => void;
};
...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  if (pool.length === 0) {
    return coords.map((c) => ({ ...c, src: '', alt: '', id: '', originalIndex: -1 }));
  }

  const normalizedImages = pool.map((image, idx) => {
    if (typeof image === 'string') {
      return { src: image, alt: '', id: '', originalIndex: idx };
    }
    return { src: image.src || '', alt: image.alt || '', id: image.id || '', originalIndex: idx };
  });
  ...
  return coords.map((c, i) => ({
    ...
    id: usedImages[i].id,
    originalIndex: usedImages[i].originalIndex,
  }));
}
...
export default function DomeGallery({
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    // Fire onItemClick callback if provided
    const itemIndex = parseInt(parent.dataset.itemIndex || '-1', 10);
    if (onItemClick && itemIndex >= 0 && itemIndex < items.length) {
      onItemClick(items[itemIndex].originalIndex, items[itemIndex]);
    }
          ...
          <div className="stage">
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                ...
                >
                  <div
                    className="item__image absolute block overflow-hidden cursor-pointer bg-zinc-800/90 transition-transform duration-300"
                    role="button"
                    tabIndex={0}
...
export type { ImageItem, ItemDef, DomeGalleryProps };
