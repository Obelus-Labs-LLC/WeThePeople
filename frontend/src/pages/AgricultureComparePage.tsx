...
type ListRow = AgricultureCompanyListItem & { id: string; name: string };
type ComparedRow = AgricultureComparisonItem & { id: string; name: string };
...
export default function AgricultureComparePage() {
  ...
  useEffect(() => {
    ...
    getAgricultureCompanies({ limit: 200 })
      ...
      .catch((err) => { console.warn('[AgricultureComparePage] fetch failed:', err); })
      .finally(() => {
  ...
  return (
    <SectorCompareLayout<ListRow, ComparedRow>
      config={config}
      title="Agriculture comparison"
      subtitle="Side-by-side comparison of contracts, lobbying, enforcement, and financials across agriculture companies."
