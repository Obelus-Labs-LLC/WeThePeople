...
function DirectoryCompanyCard({ company, index }: { company: ChemicalCompanyListItem; index: number }) {
  ...
  return (
    ...
    >
      <Link
        to={`/chemicals/${company.company_id}`}
    ...
    getChemicalsCompanies({ limit: 200 })
      .then((res) => {
        if (!Array.isArray(res.companies)) {
          throw new Error('Chemicals sector data is not yet available.');
        }
        ...
        >
          <div className="relative max-w-[480px] w-full mb-4">
            ...
            <input
              ...
              className="w-full rounded-xl border border-[#0A0A0A] bg-[#111111] py-3 pl-12 pr-4 font-body text-lg text-white placeholder:text-white/30 outline-none transition-colors focus:border-teal-500/50"
            />
          </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center h-64">
            <div className="flex flex-col items-center gap-3">
              <div className="h-8 w-8 border-2 border-white/20 border-t-teal-500 rounded-full animate-spin" />
