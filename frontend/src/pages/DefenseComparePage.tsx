...
type ListRow = DefenseCompanyListItem & { id: string; name: string };
type ComparedRow = DefenseComparisonItem & { id: string; name: string };
...
export default function DefenseComparePage() {
  ...
  useEffect(() => {
    ...
    getDefenseCompanies({ limit: 200 })
      ...
      .catch((err) => { console.warn('[DefenseComparePage] fetch failed:', err); })
      .finally(() => {
  ...
  return (
    <SectorCompareLayout<ListRow, ComparedRow>
      ...
      subtitle="Side-by-side comparison of contracts, lobbying, enforcement, and market data across defense companies."
      dataCredit="USASPENDING + SENATE LDA + DOD"
      footerNote="Contracts via USASpending. Lobbying via Senate LDA. Enforcement via DOD IG."
