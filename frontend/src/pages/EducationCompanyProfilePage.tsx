...
import {
  getEducationCompanyDetail,
  getEducationCompanyContracts,
  getEducationCompanyContractSummary,
  getEducationCompanyLobbying,
  getEducationCompanyLobbySummary,
  getEducationCompanyEnforcement,
  getEducationCompanyFilings,
  getEducationCompanyDonations,
  getEducationCompanyStock,
  type EducationCompanyDetail,
  type EducationContractItem,
  type EducationContractSummary,
  type EducationLobbyingItem,
  type EducationLobbySummary,
  type EducationEnforcementItem,
  type EducationFilingItem,
  type EducationDonationItem,
  type EducationStockData,
} from '../api/education';
...
function MetricCard({ label, value, icon: Icon, color = '#A855F7' }: { label: string; value: string; icon: LucideIcon; color?: string }) {
  return (
...
export default function EducationCompanyProfilePage() {
  const { companyId } = useParams<{ companyId: string }>();
  const [activeTab, setActiveTab] = useState<TabKey>('overview');

  const [detail, setDetail] = useState<EducationCompanyDetail | null>(null);
  const [stock, setStock] = useState<EducationStockData['latest_stock']>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Lazy-loaded tab data
  const [contracts, setContracts] = useState<EducationContractItem[]>([]);
  const [contractTotal, setContractTotal] = useState(0);
  const [contractSummary, setContractSummary] = useState<EducationContractSummary | null>(null);
  const [contractsLoaded, setContractsLoaded] = useState(false);

  const [lobbying, setLobbying] = useState<EducationLobbyingItem[]>([]);
  const [lobbyTotal, setLobbyTotal] = useState(0);
  const [lobbySummary, setLobbySummary] = useState<EducationLobbySummary | null>(null);
  const [lobbyingLoaded, setLobbyingLoaded] = useState(false);

  const [enforcement, setEnforcement] = useState<EducationEnforcementItem[]>([]);
  ...
  const [donations, setDonations] = useState<EducationDonationItem[]>([]);
  const [donationTotal, setDonationTotal] = useState(0);
  const [donationsLoaded, setDonationsLoaded] = useState(false);

  const [filings, setFilings] = useState<EducationFilingItem[]>([]);
  ...
  useEffect(() => {
    ...
    setError(null);
    Promise.all([
      getEducationCompanyDetail(companyId),
      getEducationCompanyStock(companyId).catch(() => ({ latest_stock: null })),
    ])
      .then(([d, s]) => {
        if (cancelled) return;
        setDetail(d);
  ...
  useEffect(() => {
    ...
    if (activeTab === 'contracts' && !contractsLoaded) {
      Promise.all([
        getEducationCompanyContracts(companyId, { limit: 100 }),
        getEducationCompanyContractSummary(companyId).catch(() => null),
      ])
        .then(([c, s]) => {
    ...
    if (activeTab === 'lobbying' && !lobbyingLoaded) {
      Promise.all([
        getEducationCompanyLobbying(companyId, { limit: 100 }),
        getEducationCompanyLobbySummary(companyId).catch(() => null),
      ])
        .then(([l, s]) => {
    ...
    if (activeTab === 'enforcement' && !enforcementLoaded) {
      getEducationCompanyEnforcement(companyId, { limit: 100 })
        .then((r) => {
    ...
    if (activeTab === 'donations' && !donationsLoaded) {
      getEducationCompanyDonations(companyId, { limit: 100 })
        .then((r) => {
    ...
    if (activeTab === 'filings' && !filingsLoaded) {
      getEducationCompanyFilings(companyId, { limit: 100 })
  ...
  if (error || !detail) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        ...
        <Link to="/education/companies" className="font-body text-sm text-white/50 hover:text-white no-underline">
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-3">
              ...
              <WatchlistButton entityType="company" entityId={detail.company_id || companyId || ""} entityName={detail.display_name} sector="education" />
            </div>
            <div className="mt-2 flex items-center gap-3 flex-wrap">
              {detail.ticker && (
                <span className="rounded bg-purple-500/20 px-3 py-1 font-mono text-sm font-bold text-purple-400">
                  {detail.ticker}
                </span>
              )}
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                {detail.enforcement_count > 0 && (
                  <button onClick={() => setActiveTab('enforcement')} className="cursor-pointer rounded-xl border border-white/10 bg-white/[0.03] p-5 text-left transition-colors hover:bg-white/[0.06]">
                    ...
                    <p className="font-body text-xs text-white/40 mt-1">Dept. of Education enforcement data</p>
                  </button>
                )}
              </div>
