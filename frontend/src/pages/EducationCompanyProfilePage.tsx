...
import {
  FileText, Landmark, Scale, AlertTriangle, Heart,
} from 'lucide-react';
import {
  SectorProfileLayout,
  ProfileSection,
...
function partyAccent(party?: string | null): string {
  if (party === 'DEM') return 'var(--color-dem)';
  if (party === 'REP') return 'var(--color-rep)';
...
export default function EducationCompanyProfilePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getEducationCompanyStock(companyId).catch(() => ({ latest_stock: null } as EducationStockData)),
    ])
      .then(([d, s]) => {
        if (cancelled) return;
        setDetail(d);
  ...
  const tabs = [
    ...
    },
    {
      ...
      count: donationTotal,
      render: () => (
        <ProfileSection title="Political Donations" icon={Heart} count={donationTotal} accent={accent}>
          <ProfileRecordList
            ...
            records={donations.map((d) => (
              <ProfileRecordCard
                key={d.id}
                accent={accent}
                title={d.recipient_name || 'Political Donation'}
                amount={d.amount != null ? fmtDollar(d.amount) : undefined}
                amountAccent="var(--color-green)"
                meta={<ProfileRecordMeta items={[
                  ...(d.party ? [{ value: d.party, accent: partyAccent(d.party) }] : []),
                  ...(d.contributor_name ? [{ value: d.contributor_name }] : []),
                  ...(d.date ? [{ value: fmtDate(d.date) }] : []),
                ]} />}
              />
            ))}
          />
  ...
  return (
    <SectorProfileLayout
      ...
      companiesPath="/education/companies"
    />
  );
}
