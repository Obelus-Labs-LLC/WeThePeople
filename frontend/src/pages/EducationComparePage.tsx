...
type ListRow = EducationCompanyListItem & { id: string; name: string };
type ComparedRow = EducationComparisonItem & { id: string; name: string };
...
export default function EducationComparePage() {
  ...
  useEffect(() => {
    ...
    getEducationCompanies({ limit: 200 })
      ...
      .catch((err) => { console.warn('[EducationComparePage] fetch failed:', err); })
      .finally(() => {
  ...
  return (
    <SectorCompareLayout<ListRow, ComparedRow>
      ...
      subtitle="Side-by-side comparison of contracts, lobbying, enforcement, and financials across education companies."
      dataCredit="USASPENDING + SENATE LDA + ED.GOV"
      footerNote="Contracts via USASpending. Lobbying via Senate LDA. Enforcement via Dept of Education."
