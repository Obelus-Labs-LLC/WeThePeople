...
import {
  ...
  getEducationComparison,
  type EducationCompanyListItem,
  type EducationComparisonItem,
} from '../api/education';
  ...
  return (
    <div ref={ref} className="relative flex-1 min-w-0">
      ...
      <button
        ...
        aria-expanded={open}
        className="flex w-full items-center justify-between gap-2 rounded-lg border border-white/10 bg-white/[0.05] px-4 py-3 text-left transition-all hover:border-white/20 focus:outline-none focus:border-purple-500/50"
      >
        <span className={`font-body text-sm truncate ${selected ? 'text-white' : 'text-white/40'}`}>
          {selected ? (
            <>
              {selected.display_name}
              {selected.ticker && <span className="ml-2 font-mono text-xs text-purple-400">{selected.ticker}</span>}
            </>
          ) : (
            'Select company\u2026'
          )}
        ...
        <div className="absolute top-full left-0 right-0 z-50 mt-1 rounded-lg border border-white/10 bg-[#111111] shadow-2xl max-h-72 overflow-hidden flex flex-col">
          <div className="p-2 border-b border-white/5">
            <input
              ...
              autoFocus
              className="w-full rounded-md bg-white/[0.05] px-3 py-2 font-body text-sm text-white placeholder:text-white/40 outline-none border border-white/5 focus:border-purple-500/30"
            />
          </div>
              ...
              filtered.map((co) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between px-4 py-2.5 text-left transition-colors hover:bg-white/[0.05] ${
                    co.company_id === value ? 'bg-purple-500/10' : ''
                  }`}
                >
                  <span className="font-body text-sm text-white truncate">{co.display_name}</span>
                  {co.ticker && (
                    <span className="font-mono text-[11px] text-purple-400 shrink-0 ml-3">{co.ticker}</span>
                  )}
                </button>
              ))
            )}
...
function getWinner(metric: Metric, a: EducationComparisonItem, b: EducationComparisonItem): 'a' | 'b' | null {
...
export default function EducationComparePage() {
  const [allCompanies, setAllCompanies] = useState<EducationCompanyListItem[]>([]);
  const [idA, setIdA] = useState('');
  const [idB, setIdB] = useState('');
  const [compared, setCompared] = useState<EducationComparisonItem[]>([]);
  ...
  function handleCompare() {
    ...
    setComparing(true);
    getEducationComparison([idA, idB])
      ...
      <div className="mx-auto max-w-[1400px] px-4 py-6 lg:px-16 lg:py-14">
        ...
        <div className="flex items-end justify-between mb-6 animate-fade-up">
          <div>
            ...
            <p className="mt-1 font-body text-lg text-white/50">
              Side-by-side comparison of contracts, lobbying, and enforcement
            </p>
          </div>
          <div className="hidden sm:block text-right">
            <p className="font-mono text-[11px] text-white/30">
              DATA: <span className="text-white/50">USASPENDING + SENATE LDA + DEPT. OF ED</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 mb-8">
          ...
          <button
            ...
            className="shrink-0 flex items-center justify-center gap-2 rounded-lg bg-purple-500 px-6 py-3 font-heading text-sm font-bold uppercase tracking-wider text-[#0a0a0f] transition-all hover:bg-purple-400 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {comparing ? (
          ...
          <>
                  ...
                  <div className="flex items-center gap-4 p-5">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {co.ticker && (
                          <span className="rounded-full bg-purple-500/10 px-2.5 py-0.5 font-mono text-[11px] font-bold text-purple-400 shrink-0">
                            {co.ticker}
                          </span>
                        )}
                      </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className={`text-center font-mono text-sm font-semibold ${
                              winner === 'a' ? 'text-purple-400' : 'text-white'
                            }`}>
                              {metric.format(valA)}
                            </span>
                            <span className={`text-center font-mono text-sm font-semibold ${
                              winner === 'b' ? 'text-purple-400' : 'text-white'
                            }`}>
                              {metric.format(valB)}
                            </span>
                          </div>
            ...
            <div className="mt-8 pt-4 border-t border-white/5 flex items-center justify-between">
              <p className="font-mono text-[10px] text-white/40">
                Contracts via USASpending. Lobbying via Senate LDA. Enforcement via Dept. of Education.
              </p>
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1.5 font-mono text-[10px] text-white/40">
                  <span className="h-2 w-2 rounded-full bg-purple-400" /> Winner
                </span>
              </div>
            </div>
          </>
