...
function DirectoryCompanyCard({ company, index }: { company: EnergyCompanyListItem; index: number }) {
        ...
        >
          <div className="relative flex h-full flex-col p-6 overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-3 border-t border-white/10 pt-4">
              <div>
                <p className="font-mono text-xs text-white/40 mb-1">EMISSIONS</p>
                <p className="font-mono text-lg text-white">
                  {company.emission_count.toLocaleString()}
                </p>
              </div>
              <div>
    ...
    getEnergyCompanies({ limit: 200 })
      .then((res) => {
        if (!Array.isArray(res.companies)) {
          throw new Error('Energy sector data is not yet available.');
        }
        ...
        >
          <div className="relative max-w-[480px] w-full mb-4">
            ...
            <input
              ...
              className="w-full rounded-xl border border-[#0A0A0A] bg-[#111111] py-3 pl-12 pr-4 font-body text-lg text-white placeholder:text-white/30 outline-none transition-colors focus:border-orange-500/50"
            />
          </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center h-64">
            <div className="flex flex-col items-center gap-3">
              <div className="h-8 w-8 border-2 border-white/20 border-t-orange-500 rounded-full animate-spin" />
