...
const METRICS: Metric[] = [
  ...
  { label: 'Total Emissions (CO2e)', key: 'total_emissions', format: fmtEmissions },
...
function bestIdx(items: EnergyComparisonItem[], key: string, higher: boolean): number {
  let best = -1;
...
const HIGHER_IS_BETTER = new Set([
  'market_cap', 'contract_count', 'total_contract_value', 'profit_margin',
]);
const LOWER_IS_BETTER = new Set([
  'total_emissions', 'lobbying_total', 'enforcement_count', 'total_penalties',
]);
...
export default function EnergyComparePage() {
  ...
  useEffect(() => {
    ...
    setComparing(true);
    getEnergyComparison(selectedIds)
    ...
    <div className="min-h-screen">
      <div className="flex flex-col px-8 py-6 lg:px-12">
        ...
        <div className="mb-6 shrink-0">
          <EnergySectorHeader />
          <div className="mb-2">
            <BackButton to="/energy" label="Energy Dashboard" />
          </div>
        ...
        <div className="mb-5 shrink-0">
          <div className="relative max-w-sm w-full mb-3">
            ...
            <input
              ...
              className="w-full rounded-lg border border-zinc-800 bg-[#18181B] py-2 pl-9 pr-4 font-body text-sm text-zinc-100 placeholder:text-zinc-500 outline-none focus:border-orange-500/50 transition-colors"
            />
          </div>
            ...
            {filteredCompanies.map((co) => {
              ...
              return (
                ...
                >
                  {co.display_name}
                  {co.ticker && (
                    <span className={`ml-1.5 font-mono text-[10px] ${selected ? 'text-orange-400' : 'text-zinc-600'}`}>
                      {co.ticker}
                    </span>
                  )}
                </button>
            ...
            <div style={{ minWidth: 800 }}>
              ...
              >
                ...
                {compared.map((co) => (
                  <div
                    key={co.company_id}
                    className="bg-[#18181B] p-4 text-center border-b border-orange-500/30"
                  >
                    ...
                    {co.ticker && (
                      <span className="font-mono text-xs text-orange-400">{co.ticker}</span>
                    )}
                  </div>
                ))}
              </div>
