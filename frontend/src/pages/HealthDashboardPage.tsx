...
import {
  Building, Activity, AlertTriangle, FlaskConical, DollarSign,
  Search, Building2, ArrowLeft,
  type LucideIcon,
} from 'lucide-react';
...
function VitalCard({
  ...
  const borderColor = critical ? '#DC2626' : '#3B82F6';
  const iconColor = critical ? '#DC2626' : '#3B82F6';
...
function SectorBreakdown({ bySector }: { bySector: Record<string, number> }) {
  ...
  return (
    <div className="bg-white/[0.05] backdrop-blur-sm rounded-xl border border-white/10 p-6 shrink-0">
        ...
        {segments.map((seg) => (
          <div
            ...
            style={{
              ...
              minWidth: seg.pct > 8 ? undefined : '0px',
            }}
          >
            {seg.pct > 10 && (
              <span
                className="text-xs font-bold text-white px-3 truncate"
                style={{ fontFamily: "'JetBrains Mono', monospace" }}
              >
                {seg.label}
              </span>
      ...
      {/* Axis labels */}
      <div
        className="flex justify-between mt-2 text-white/40"
      ...
      >
        <table className="w-full border-collapse">
          <thead className="sticky top-0 z-10 bg-[rgba(10,10,15,0.9)] backdrop-blur-sm" style={{ boxShadow: '0 1px 0 rgba(255,255,255,0.1)' }}>
            <tr>
              {['COMPANY', 'SECTOR', 'ADVERSE EVENTS', 'RECALLS'].map((h, i) => (
                <th
                  key={h}
              ...
              >
                <td className="p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      {c.ticker && (
                        <span
                          className="inline-block mt-0.5 rounded px-2 py-0.5 text-xs bg-white/[0.05] text-white/50"
                          style={{ fontFamily: "'JetBrains Mono', monospace" }}
                        >
                          {c.ticker}
                        </span>
                ...
                <td className="p-3">
                  <span
                    ...
                    style={{ fontFamily: "'JetBrains Mono', monospace", color: SECTOR_COLORS[c.sector_type] || '#64748B' }}
                  >
                    {SECTOR_LABELS[c.sector_type] || c.sector_type}
                  </span>
                </td>
                <td className="p-3 text-right">
                  <span className="text-sm font-bold" style={{ fontFamily: "'JetBrains Mono', monospace", color: '#DC2626' }}>
                ...
                <td className="p-3 text-right">
                  {c.recall_count > 0 ? (
                    <span
                      ...
                      style={{ background: 'rgba(220,38,38,0.15)', color: '#FCA5A5', fontFamily: "'JetBrains Mono', monospace" }}
                    >
                      {c.recall_count}
                    </span>
                  ) : (
                    <span className="text-sm text-white/20" style={{ fontFamily: "'JetBrains Mono', monospace" }}>0</span>
                  )}
                </td>
              </tr>
            ))}
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1600px] flex flex-col px-8 py-8 md:px-12 md:py-10">
        ...
        <div className="flex items-end justify-between pb-6 mb-8 shrink-0 border-b border-white/10">
          <div>
            ...
            >
              Tracking adverse events, FDA recalls, clinical trials, and physician payments
              across pharma, biotech, insurers, pharmacies, and healthcare distributors.
            </p>
          </div>
          <span
        ...
        <div className="grid grid-cols-12 gap-6">
          <div
            className="col-span-12 md:col-span-4 flex flex-col gap-6 pr-2"
          >
            {stats && (
              <>
                <VitalCard title="Tracked Companies" value={fmtNum(stats.total_companies)} icon={Building} delay={0} />
                <VitalCard title="Adverse Events" value={fmtNum(stats.total_adverse_events)} icon={AlertTriangle} critical delay={0.1} />
                <VitalCard title="FDA Recalls" value={fmtNum(stats.total_recalls)} icon={Activity} critical delay={0.2} />
                <VitalCard title="Clinical Trials" value={fmtNum(stats.total_trials)} icon={FlaskConical} delay={0.3} />
                <VitalCard title="Physician Payments" value={fmtNum(stats.total_payments)} icon={DollarSign} delay={0.4} />
              </>
            )}
          </div>
        ...
        <div className="border-t border-white/10 pt-6 mt-8">
          ...
          <div className="flex flex-wrap gap-x-8 gap-y-3 mt-4">
            {['OpenFDA', 'ClinicalTrials.gov', 'CMS Open Payments', 'SEC EDGAR', 'Yahoo Finance'].map((source) => (
