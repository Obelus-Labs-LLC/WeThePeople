  ...
  return (
    <div ref={ref} className="relative flex-1 min-w-0">
      ...
      <button
        ...
        aria-expanded={open}
        className="flex w-full items-center justify-between gap-2 rounded-lg border border-white/10 bg-white/[0.05] px-4 py-3 text-left transition-all hover:border-white/20 focus:outline-none focus:border-[#8B5CF6]/50"
      >
        <span className={`font-body text-sm truncate ${selected ? 'text-white' : 'text-white/40'}`}>
          {selected ? (
            <>
              {selected.display_name}
              {selected.ticker && <span className="ml-2 font-mono text-xs text-[#8B5CF6]">{selected.ticker}</span>}
            </>
          ) : (
            'Select company\u2026'
          )}
        ...
        <div className="absolute top-full left-0 right-0 z-50 mt-1 rounded-lg border border-white/10 bg-[#111111] shadow-2xl max-h-72 overflow-hidden flex flex-col">
          <div className="p-2 border-b border-white/5">
            <input
              ...
              autoFocus
              className="w-full rounded-md bg-white/[0.05] px-3 py-2 font-body text-sm text-white placeholder:text-white/40 outline-none border border-white/5 focus:border-[#8B5CF6]/30"
            />
          </div>
              ...
              filtered.map((co) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between px-4 py-2.5 text-left transition-colors hover:bg-white/[0.05] ${
                    co.company_id === value ? 'bg-[#8B5CF6]/10' : ''
                  }`}
                >
                  <span className="font-body text-sm text-white truncate">{co.display_name}</span>
                  {co.ticker && (
                    <span className="font-mono text-[11px] text-[#8B5CF6] shrink-0 ml-3">{co.ticker}</span>
                  )}
                </button>
              ))
            )}
...
function getWinner(metric: Metric, a: TechComparisonItem, b: TechComparisonItem): 'a' | 'b' | null {
        ...
        <div className="flex items-end justify-between mb-6 animate-fade-up">
          ...
          <div className="hidden sm:block text-right">
            <p className="font-mono text-[11px] text-white/30">
              DATA: <span className="text-white/50">USPTO + USASPENDING + SENATE LDA + SEC</span>
            </p>
          </div>
        </div>
          ...
          <>
                  ...
                  <div className="flex items-center gap-4 p-5">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {co.ticker && (
                          <span className="rounded-full bg-[#8B5CF6]/10 px-2.5 py-0.5 font-mono text-[11px] font-bold text-[#8B5CF6] shrink-0">
                            {co.ticker}
                          </span>
                        )}
                      </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className={`text-center font-mono text-sm font-semibold ${
                              winner === 'a' ? 'text-[#8B5CF6]' : 'text-white'
                            }`}>
                              {metric.format(valA)}
                            </span>
                            <span className={`text-center font-mono text-sm font-semibold ${
                              winner === 'b' ? 'text-[#8B5CF6]' : 'text-white'
                            }`}>
                              {metric.format(valB)}
                            </span>
                          </div>
            ...
            <div className="mt-8 pt-4 border-t border-white/5 flex items-center justify-between">
              <p className="font-mono text-[10px] text-white/40">
                Patents via USPTO. Contracts via USASpending. Lobbying via Senate LDA. Filings via SEC EDGAR.
              </p>
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1.5 font-mono text-[10px] text-white/40">
                  <span className="h-2 w-2 rounded-full bg-[#8B5CF6]" /> Winner
                </span>
              </div>
            </div>
          </>
