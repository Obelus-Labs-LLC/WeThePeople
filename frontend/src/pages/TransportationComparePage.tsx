...
function CompanyDropdown({
  label, value, onChange, companies, excludeId,
}: {
  label: string; value: string; onChange: (id: string) => void;
  companies: TransportationCompanyListItem[]; excludeId?: string;
}) {
  ...
  return (
    <div ref={ref} className="relative flex-1 min-w-0">
      <span className="block font-heading text-[10px] font-semibold tracking-wider text-white/40 uppercase mb-1.5">{label}</span>
      <button onClick={() => setOpen(!open)}
        onKeyDown={(e) => { if (e.key === 'Escape') setOpen(false); if ((e.key === 'Enter' || e.key === ' ') && !open) { e.preventDefault(); setOpen(true); } if (e.key === 'ArrowDown' && !open) { e.preventDefault(); setOpen(true); } }}
        aria-haspopup="listbox" aria-expanded={open}
        className="flex w-full items-center justify-between gap-2 rounded-lg border border-white/10 bg-white/[0.05] px-4 py-3 text-left transition-all hover:border-white/20 focus:outline-none focus:border-[#3B82F6]/50">
        <span className={`font-body text-sm truncate ${selected ? 'text-white' : 'text-white/40'}`}>
          {selected ? (<>{selected.display_name}{selected.ticker && <span className="ml-2 font-mono text-xs text-[#3B82F6]">{selected.ticker}</span>}</>) : ('Select company\u2026')}
        </span>
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 z-50 mt-1 rounded-lg border border-white/10 bg-[#111111] shadow-2xl max-h-72 overflow-hidden flex flex-col">
          <div className="p-2 border-b border-white/5">
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name or ticker\u2026" autoFocus
              className="w-full rounded-md bg-white/[0.05] px-3 py-2 font-body text-sm text-white placeholder:text-white/40 outline-none border border-white/5 focus:border-[#3B82F6]/30" />
          </div>
          <div className="overflow-y-auto" role="listbox">
            {filtered.length === 0 ? (
              ...
              filtered.map((co) => (
                <button key={co.company_id} role="option" aria-selected={co.company_id === value}
                  onClick={() => { onChange(co.company_id); setOpen(false); setSearch(''); }}
                  className={`flex w-full items-center justify-between px-4 py-2.5 text-left transition-colors hover:bg-white/[0.05] ${co.company_id === value ? 'bg-[#3B82F6]/10' : ''}`}>
                  <span className="font-body text-sm text-white truncate">{co.display_name}</span>
                  {co.ticker && <span className="font-mono text-[11px] text-[#3B82F6] shrink-0 ml-3">{co.ticker}</span>}
                </button>
              ))
            )}
          </div>
...
function getWinner(metric: Metric, a: TransportationComparisonItem, b: TransportationComparisonItem): 'a' | 'b' | null {
...
export default function TransportationComparePage() {
  const [allCompanies, setAllCompanies] = useState<TransportationCompanyListItem[]>([]);
  const [idA, setIdA] = useState('');
  const [idB, setIdB] = useState('');
  const [compared, setCompared] = useState<TransportationComparisonItem[]>([]);
    ...
    <div className="min-h-screen">
      <div className="mx-auto max-w-[1400px] px-4 py-6 lg:px-16 lg:py-14">
        ...
        <div className="flex items-end justify-between mb-6 animate-fade-up">
          <div>
            <h1 className="font-heading text-3xl sm:text-4xl font-bold uppercase tracking-wide text-white xl:text-6xl">Compare</h1>
            <p className="mt-1 font-body text-lg text-white/50">Side-by-side comparison of contracts, lobbying, and enforcement</p>
          </div>
          <div className="hidden sm:block text-right">
            <p className="font-mono text-[11px] text-white/30">DATA: <span className="text-white/50">USASPENDING + SENATE LDA + FEDERAL REGISTER</span></p>
          </div>
        </div>

        {/* Selector bar */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 mb-8">
          <CompanyDropdown label="Company A" value={idA} onChange={setIdA} companies={allCompanies} excludeId={idB} />
          ...
          <CompanyDropdown label="Company B" value={idB} onChange={setIdB} companies={allCompanies} excludeId={idA} />
          <button onClick={handleCompare} disabled={!idA || !idB || idA === idB || comparing}
            className="shrink-0 flex items-center justify-center gap-2 rounded-lg bg-[#3B82F6] px-6 py-3 font-heading text-sm font-bold uppercase tracking-wider text-white transition-all hover:bg-[#3B82F6]/90 disabled:opacity-40 disabled:cursor-not-allowed">
            {comparing ? <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> : <GitCompareArrows size={16} />}
            Compare
          </button>
        </div>
        ...
        {compared.length >= 2 && coA && coB ? (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
              {[coA, coB].map((co) => (
                <SpotlightCard key={co.company_id} className="rounded-xl border border-white/10 bg-white/[0.03]" spotlightColor="rgba(59, 130, 246, 0.1)">
                  <div className="flex items-center gap-4 p-5">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-4 text-white/40">
                        {co.sector_type && <span className="font-body text-xs capitalize">{co.sector_type.replace(/_/g, ' ')}</span>}
                      </div>
                    </div>
                  </div>
                </SpotlightCard>
                ...
                return (
                  <div key={group.title}>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h2 className="font-heading text-xs font-bold uppercase tracking-wider text-white/50">{group.title}</h2>
                    ...
                    <div>
                      {group.metrics.map((metric, idx) => {
                        ...
                        return (
                          <div key={metric.key}
                            className={`grid grid-cols-[1fr_1fr_1fr] items-center py-3 border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors ${idx % 2 === 0 ? 'bg-white/[0.01]' : ''}`}>
                            <span className="font-body text-sm text-white/50 pl-1">{metric.label}</span>
                            <span className={`text-center font-mono text-sm font-semibold ${winner === 'a' ? 'text-[#3B82F6]' : 'text-white'}`}>{metric.format(valA)}</span>
                            <span className={`text-center font-mono text-sm font-semibold ${winner === 'b' ? 'text-[#3B82F6]' : 'text-white'}`}>{metric.format(valB)}</span>
                          </div>
                        );
                      })}
                    </div>
            ...
            <div className="mt-8 pt-4 border-t border-white/5 flex items-center justify-between">
              <p className="font-mono text-[10px] text-white/40">Contracts via USASpending. Lobbying via Senate LDA. Enforcement via Federal Register.</p>
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1.5 font-mono text-[10px] text-white/40"><span className="h-2 w-2 rounded-full bg-[#3B82F6]" /> Winner</span>
              </div>
            </div>
          </>
        ) : !comparing ? (
          <div className="flex items-center justify-center py-20">
            <div className="text-center">
              ...
              <p className="font-body text-sm text-white/40">Pick two companies above and hit Compare</p>
            </div>
          </div>
        ) : null}
