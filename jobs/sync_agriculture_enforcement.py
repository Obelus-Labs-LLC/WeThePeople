...
def sync_company_enforcement(session, company: TrackedAgricultureCompany) -> int:
    """Sync enforcement actions for a single agriculture company."""
    ...
    for search_name in search_names:
        for source_label, agency_slug in AGENCY_SLUGS.items():
            ...
            for r in results:
                ...
                if session.query(AgricultureEnforcement).filter_by(dedupe_hash=dedupe).first():
                    continue

                session.add(AgricultureEnforcement(
...
def main():
    ...
    if args.company:
        company = session.query(TrackedAgricultureCompany).filter_by(company_id=args.company).first()
        if not company:
            log.error(f"Company '{args.company}' not found")
            return
        companies = [company]
    else:
        companies = session.query(TrackedAgricultureCompany).all()

    log.info(f"Syncing enforcement for {len(companies)} agriculture companies...")
    total = 0
