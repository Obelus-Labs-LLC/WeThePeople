"""
Chemicals sector enforcement data ingestion.

Fetches enforcement actions from:
- EPA (Environmental Protection Agency) — via Federal Register API
- OSHA (Occupational Safety and Health Administration) — via Federal Register API
- DOJ (Department of Justice) — via Federal Register API

Usage:
    python jobs/sync_chemicals_enforcement.py [--company COMPANY_ID]
"""
...
from models.chemicals_models import TrackedChemicalCompany, ChemicalEnforcement
...
def sync_company_enforcement(session, company: TrackedChemicalCompany) -> int:
    """Sync enforcement actions for a single chemical company."""
    ...
    for search_name in search_names:
        for source_label, agency_slug in AGENCY_SLUGS.items():
            ...
            for r in results:
                ...
                if session.query(ChemicalEnforcement).filter_by(dedupe_hash=dedupe).first():
                    continue

                session.add(ChemicalEnforcement(
...
def main():
    ...
    if args.company:
        company = session.query(TrackedChemicalCompany).filter_by(company_id=args.company).first()
        if not company:
            log.error(f"Company '{args.company}' not found")
            return
        companies = [company]
    else:
        companies = session.query(TrackedChemicalCompany).all()

    log.info(f"Syncing enforcement for {len(companies)} chemical companies...")
    total = 0
