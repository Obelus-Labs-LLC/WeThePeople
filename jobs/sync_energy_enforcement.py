"""
Energy sector enforcement data ingestion.

Fetches enforcement actions from:
- EPA (Environmental Protection Agency) — via Federal Register API
- FERC (Federal Energy Regulatory Commission) — via Federal Register API
- DOJ (Department of Justice) — via Federal Register API

Usage:
    python jobs/sync_energy_enforcement.py [--company COMPANY_ID]
"""
...
from models.energy_models import TrackedEnergyCompany, EnergyEnforcement
...
def sync_company_enforcement(session, company: TrackedEnergyCompany) -> int:
    """Sync enforcement actions for a single energy company."""
    ...
    for search_name in search_names:
        for source_label, agency_slug in AGENCY_SLUGS.items():
            ...
            for r in results:
                ...
                if session.query(EnergyEnforcement).filter_by(dedupe_hash=dedupe).first():
                    continue

                session.add(EnergyEnforcement(
...
def main():
    ...
    if args.company:
        company = session.query(TrackedEnergyCompany).filter_by(company_id=args.company).first()
        if not company:
            log.error(f"Company '{args.company}' not found")
            return
        companies = [company]
    else:
        companies = session.query(TrackedEnergyCompany).all()

    log.info(f"Syncing enforcement for {len(companies)} energy companies...")
    total = 0
