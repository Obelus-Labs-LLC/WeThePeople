"""
Telecom sector data ingestion job.
...
Usage:
    python jobs/sync_telecom_data.py [--company COMPANY_ID] [--skip-sec] [--skip-contracts] [--skip-lobbying]
"""
...
log = logging.getLogger("sync_telecom")
...
def fetch_sec_filings(session, company: TrackedTelecomCompany, limit: int = 10000):
    """Fetch recent SEC filings for a telecom company."""
...
def fetch_contracts(session, company: TrackedTelecomCompany):
    ...
    while True:
        ...
        for r in results:
            ...
            if session.query(TelecomGovernmentContract).filter_by(dedupe_hash=dedupe).first():
                continue

            session.add(TelecomGovernmentContract(
                company_id=company.company_id,
                award_id=award_id,
...
def fetch_lobbying(session, company: TrackedTelecomCompany):
    ...
    for year in years:
        ...
        while page_url:
            ...
            for r in results:
                ...
                if session.query(TelecomLobbyingRecord).filter_by(dedupe_hash=dedupe).first():
                    continue
                ...
                session.add(TelecomLobbyingRecord(
                    company_id=company.company_id,
...
def main():
    parser = argparse.ArgumentParser(description="Sync telecom sector data")
    ...
    try:
        ...
        query = session.query(TrackedTelecomCompany).filter(TrackedTelecomCompany.is_active == 1)
        if args.company:
            query = query.filter(TrackedTelecomCompany.company_id == args.company)
        companies = query.all()

        log.info(f"Syncing {len(companies)} telecom companies...")
