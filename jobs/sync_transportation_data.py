"""
Transportation sector data ingestion job.
...
Usage:
    python jobs/sync_transportation_data.py [--company COMPANY_ID] [--skip-sec] [--skip-contracts] [--skip-lobbying]
"""
...
log = logging.getLogger("sync_transportation")
...
def fetch_sec_filings(session, company: TrackedTransportationCompany, limit: int = 10000):
    """Fetch recent SEC filings for a transportation company."""
    ...
    for i in range(min(len(forms), limit)):
        ...
        if session.query(SECTransportationFiling).filter_by(accession_number=acc).first():
            continue

        acc_no_dash = acc.replace("-", "")
        filing_url = f"https://www.sec.gov/Archives/edgar/data/{cik}/{acc_no_dash}/{docs[i]}" if i < len(docs) and docs[i] else None

        session.add(SECTransportationFiling(
...
def fetch_contracts(session, company: TrackedTransportationCompany):
    ...
    while True:
        ...
        for r in results:
            ...
            if session.query(TransportationGovernmentContract).filter_by(dedupe_hash=dedupe).first():
                continue

            session.add(TransportationGovernmentContract(
                company_id=company.company_id,
                award_id=award_id,
...
def main():
    parser = argparse.ArgumentParser(description="Sync transportation sector data")
    ...
    try:
        ...
        query = session.query(TrackedTransportationCompany).filter(TrackedTransportationCompany.is_active == 1)
        if args.company:
            query = query.filter(TrackedTransportationCompany.company_id == args.company)
        companies = query.all()

        log.info(f"Syncing {len(companies)} transportation companies...")
