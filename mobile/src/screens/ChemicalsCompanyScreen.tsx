...
const SECTOR_COLORS: Record<string, string> = {
  ...
  agricultural: '#10B981',
  industrial: '#0EA5E9',
  petrochemical: '#DC2626',
};

type Tab = 'overview' | 'contracts' | 'lobbying' | 'enforcement';

export default function ChemicalsCompanyScreen() {
  ...
  const loadCompany = useCallback(async () => {
    try {
      const data = await fetch(`${API_BASE}/chemicals/companies/${companyId}`).then(r => r.json());
      setCompany(data);
      setError('');
  ...
  useEffect(() => {
    fetch(`${API_BASE}/chemicals/companies/${companyId}/filings?limit=5`).then(r => r.json()).then((res) => setFilings(res.filings || [])).catch(() => {});
  }, [companyId]);
  ...
  useEffect(() => {
    if (tab === 'contracts' && contracts.length === 0 && !contractsLoading) {
      ...
      Promise.all([
        fetch(`${API_BASE}/chemicals/companies/${companyId}/contracts?limit=50`).then(r => r.json()),
        fetch(`${API_BASE}/chemicals/companies/${companyId}/contracts/summary`).then(r => r.json()),
  ...
  useEffect(() => {
    if (tab === 'lobbying' && lobbyingFilings.length === 0 && !lobbyingLoading) {
      ...
      Promise.all([
        fetch(`${API_BASE}/chemicals/companies/${companyId}/lobbying?limit=50`).then(r => r.json()),
        fetch(`${API_BASE}/chemicals/companies/${companyId}/lobbying/summary`).then(r => r.json()),
  ...
  useEffect(() => {
    if (tab === 'enforcement' && enforcementActions.length === 0 && !enforcementLoading) {
      ...
      fetch(`${API_BASE}/chemicals/companies/${companyId}/enforcement?limit=50`).then(r => r.json())
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={ACCENT} />} contentContainerStyle={{ paddingBottom: 32 }}>
        <View style={styles.header}>
          <View style={[styles.headerIcon, { backgroundColor: sectorColor + '15' }]}>
            <Ionicons name="flask" size={28} color={sectorColor} />
          </View>
  ...
  return (
    <View style={styles.tabContent}>
      <View style={styles.statsRow}>
        <View style={styles.miniStat}><Ionicons name="document-text-outline" size={18} color="#2563EB" /><Text style={styles.miniStatVal}>{company.contract_count || 0}</Text><Text style={styles.miniStatLabel}>Contracts</Text></View>
        <View style={styles.miniStat}><Ionicons name="folder-outline" size={18} color="#8B5CF6" /><Text style={styles.miniStatVal}>{company.filing_count || 0}</Text><Text style={styles.miniStatLabel}>Filings</Text></View>
        <View style={styles.miniStat}><Ionicons name="shield-outline" size={18} color="#DC2626" /><Text style={styles.miniStatVal}>{company.enforcement_count || 0}</Text><Text style={styles.miniStatLabel}>Enforcement</Text></View>
      </View>
      {company.total_contract_value > 0 && (
        <View style={styles.card}>
          <View style={styles.cardHeader}><Ionicons name="cash" size={16} color="#10B981" /><Text style={styles.cardTitle}>Government Contract Value</Text></View>
          <Text style={[styles.bigNum, { color: '#10B981' }]}>${company.total_contract_value.toLocaleString(undefined, { maximumFractionDigits: 0 })}</Text>
          <Text style={styles.cardSub}>{company.contract_count} total contracts</Text>
        </View>
      )}
      {company.latest_stock && (
        <View style={styles.card}>
          <View style={styles.cardHeader}><Ionicons name="trending-up" size={16} color="#2563EB" /><Text style={styles.cardTitle}>Stock Fundamentals</Text></View>
          <View style={styles.stockGrid}>
            {company.latest_stock.market_cap != null && <View style={styles.stockItem}><Text style={styles.stockLabel}>Market Cap</Text><Text style={styles.stockVal}>${formatLargeNum(company.latest_stock.market_cap)}</Text></View>}
            {company.latest_stock.pe_ratio != null && <View style={styles.stockItem}><Text style={styles.stockLabel}>P/E Ratio</Text><Text style={styles.stockVal}>{company.latest_stock.pe_ratio.toFixed(1)}</Text></View>}
          </View>
        </View>
      )}
      {filings.length > 0 && (
        <View style={styles.card}>
          <View style={styles.cardHeader}><Ionicons name="document" size={16} color="#8B5CF6" /><Text style={styles.cardTitle}>Recent SEC Filings</Text></View>
          {filings.map((f: any) => (
            <View key={f.id} style={styles.filingRow}>
              ...
              <View style={{ flex: 1 }}><Text style={styles.filingDesc} numberOfLines={1}>{f.description || f.accession_number}</Text>{f.filing_date && <Text style={styles.cardDate}>{f.filing_date}</Text>}</View>
            </View>
          ))}
        </View>
      )}
...
function renderContracts(contracts: any[], summary: any, loading: boolean) {
  ...
  return (
    <View style={styles.tabContent}>
      ...
      {contracts.length === 0 ? <Text style={styles.noData}>No government contracts found</Text> : contracts.map((ct: any) => (
...
function renderLobbying(filings: any[], summary: any, loading: boolean) {
  ...
  return (
    <View style={styles.tabContent}>
      ...
      {filings.length === 0 ? <Text style={styles.noData}>No lobbying filings found</Text> : filings.map((f: any) => (
...
function renderEnforcement(actions: any[], totalPenalties: number, loading: boolean) {
  ...
  return (
    <View style={styles.tabContent}>
      ...
      {actions.length === 0 ? <Text style={styles.noData}>No enforcement actions found</Text> : actions.map((a: any) => (
