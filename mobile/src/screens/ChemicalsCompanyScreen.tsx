...
const SECTOR = 'chemicals';
const log = (msg: string, err: unknown) => console.warn(`[ChemicalsCompanyScreen] ${msg}:`, err);
const ACCENT = '#F59E0B';
...
export default function ChemicalsCompanyScreen() {
  ...
  useEffect(() => {
    apiClient.getSectorCompanyFilings(SECTOR, companyId, { limit: 5 }).then((res) => setFilings(res.filings || [])).catch((e) => log('recent filings', e));
  }, [companyId]);
  ...
  useEffect(() => {
    if (tab === 'contracts' && contracts.length === 0 && !contractsLoading) {
      ...
      Promise.all([
        ...
        .catch((e) => log('fetch failed', e)).finally(() => setContractsLoading(false));
    }
  }, [tab, companyId]);
  ...
  useEffect(() => {
    if (tab === 'lobbying' && lobbyingFilings.length === 0 && !lobbyingLoading) {
      ...
      Promise.all([
        ...
        .catch((e) => log('fetch failed', e)).finally(() => setLobbyingLoading(false));
    }
  }, [tab, companyId]);
  ...
  useEffect(() => {
    if (tab === 'enforcement' && enforcementActions.length === 0 && !enforcementLoading) {
      ...
      apiClient.getSectorCompanyEnforcement(SECTOR, companyId, { limit: 50 })
        ...
        .catch((e) => log('fetch failed', e)).finally(() => setEnforcementLoading(false));
    }
  }, [tab, companyId]);
