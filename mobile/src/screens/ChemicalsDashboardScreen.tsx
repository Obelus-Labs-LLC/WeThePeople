...
export default function ChemicalsDashboardScreen() {
  ...
  const loadData = async () => {
    try {
      const [statsRes, compRes] = await Promise.all([
        fetch(`${API_BASE}/chemicals/dashboard/stats`).then(r => r.json()),
        fetch(`${API_BASE}/chemicals/companies?limit=6`).then(r => r.json()),
      ]);
      setStats(statsRes);
  ...
  if (loading) return <LoadingSpinner message="Loading chemicals data..." />;
    ...
    >
      ...
      >
        ...
        <View style={styles.heroInner}>
          <View style={styles.heroIconRow}>
            ...
            <Text style={styles.heroTitle}>Chemicals Sector</Text>
          </View>
          <Text style={styles.heroSubtitle}>
            SEC filings, contracts, lobbying, enforcement, safety data
          </Text>
        </View>
      </LinearGradient>
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('ChemicalsCompaniesDirectory')}>
        ...
        {companies.map((c: any) => (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('ChemicalsCompanyDetail', { company_id: c.company_id })}
          >
            <View style={[styles.iconWrap, { backgroundColor: (SECTOR_COLORS[c.sector_type] || '#6B7280') + '15' }]}>
              <Ionicons name="flask" size={20} color={SECTOR_COLORS[c.sector_type] || '#6B7280'} />
            </View>
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('Compare', { sector: 'chemicals' })}
      >
        <Ionicons name="git-compare" size={16} color={ACCENT} />
        <Text style={[styles.compareText, { color: ACCENT }]}>Compare Chemicals Companies</Text>
      </TouchableOpacity>

      <View style={styles.footer}>
        <Text style={styles.footerText}>Data: SEC EDGAR · EPA · USASpending.gov · Senate LDA</Text>
      </View>
    </ScrollView>
  );
}
