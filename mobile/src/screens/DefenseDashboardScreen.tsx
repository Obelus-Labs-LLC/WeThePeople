  ...
  const loadData = async () => {
    try {
      const [statsRes, compRes] = await Promise.all([
        fetch(`${API_BASE}/defense/dashboard/stats`).then(r => r.json()),
        fetch(`${API_BASE}/defense/companies?limit=6`).then(r => r.json()),
      ]);
      setStats(statsRes);
    ...
    >
      ...
      >
        ...
        <View style={styles.heroInner}>
          <View style={styles.heroIconRow}>
            ...
            <Text style={styles.heroTitle}>Defense Sector</Text>
          </View>
          <Text style={styles.heroSubtitle}>
            Military contracts, lobbying, enforcement, SEC filings
          </Text>
        </View>
      </LinearGradient>
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('DefenseCompaniesDirectory')}>
        ...
        {companies.map((c: any) => (
          ...
          >
            <View style={[styles.iconWrap, { backgroundColor: (SECTOR_COLORS[c.sector_type] || '#6B7280') + '15' }]}>
              <Ionicons name="shield-checkmark" size={20} color={SECTOR_COLORS[c.sector_type] || '#6B7280'} />
            </View>
