...
export default function EnergyDashboardScreen() {
  ...
  if (loading) return <LoadingSpinner message="Loading energy data..." />;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={styles.heroInner}>
          <View style={styles.heroIconRow}>
            ...
            <Text style={styles.heroTitle}>Energy Sector</Text>
          </View>
          <Text style={styles.heroSubtitle}>
            SEC filings, emissions, contracts, lobbying, enforcement
          </Text>
        </View>
      </LinearGradient>
        ...
        {companies.map((c: any) => (
          ...
          >
            <View style={[styles.iconWrap, { backgroundColor: (SECTOR_COLORS[c.sector_type] || '#6B7280') + '15' }]}>
              <Ionicons name="flash" size={20} color={SECTOR_COLORS[c.sector_type] || '#6B7280'} />
            </View>
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('Compare', { sector: 'energy' })}
      >
        <Ionicons name="git-compare" size={16} color={ACCENT} />
        <Text style={[styles.compareText, { color: ACCENT }]}>Compare Energy Companies</Text>
      </TouchableOpacity>

      <View style={styles.footer}>
        <Text style={styles.footerText}>Data: SEC EDGAR · EPA GHGRP · USASpending.gov · Senate LDA</Text>
      </View>
    </ScrollView>
  );
}
