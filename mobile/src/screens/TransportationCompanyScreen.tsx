...
export default function TransportationCompanyScreen() {
  ...
  const loadCompany = useCallback(async () => {
    try {
      const data = await fetch(`${API_BASE}/transportation/companies/${companyId}`).then(r => r.json());
      setCompany(data);
      setError('');
  ...
  useEffect(() => {
    fetch(`${API_BASE}/transportation/companies/${companyId}/filings?limit=5`)
      .then(r => r.json())
  ...
  useEffect(() => {
    if (tab === 'contracts' && contracts.length === 0 && !contractsLoading) {
      ...
      Promise.all([
        fetch(`${API_BASE}/transportation/companies/${companyId}/contracts?limit=50`).then(r => r.json()),
        fetch(`${API_BASE}/transportation/companies/${companyId}/contracts/summary`).then(r => r.json()),
      ])
        .then(([ctRes, sumRes]) => {
  ...
  useEffect(() => {
    if (tab === 'lobbying' && lobbyingFilings.length === 0 && !lobbyingLoading) {
      ...
      Promise.all([
        fetch(`${API_BASE}/transportation/companies/${companyId}/lobbying?limit=50`).then(r => r.json()),
        fetch(`${API_BASE}/transportation/companies/${companyId}/lobbying/summary`).then(r => r.json()),
      ])
        .then(([filRes, sumRes]) => {
  ...
  useEffect(() => {
    if (tab === 'enforcement' && enforcementActions.length === 0 && !enforcementLoading) {
      ...
      fetch(`${API_BASE}/transportation/companies/${companyId}/enforcement?limit=50`)
        .then(r => r.json())
        .then((res) => {
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={ACCENT} />} contentContainerStyle={{ paddingBottom: 32 }}>
        <View style={styles.header}>
          <View style={[styles.headerIcon, { backgroundColor: sectorColor + '15' }]}>
            <Ionicons name="car-sport" size={28} color={sectorColor} />
          </View>
    ...
    <View style={styles.tabContent}>
      ...
      {company.latest_stock && (
        <View style={styles.card}>
          ...
          {company.latest_stock.snapshot_date && <Text style={styles.cardDate}>As of {company.latest_stock.snapshot_date}</Text>}
        </View>
      )}
        ...
        actions.map((a: any) => (
          <TouchableOpacity key={a.id} style={styles.card} onPress={() => a.case_url && Linking.openURL(a.case_url)} disabled={!a.case_url}>
            ...
            {a.penalty_amount != null && a.penalty_amount > 0 && (
              <Text style={styles.penaltyAmount}>Penalty: ${a.penalty_amount.toLocaleString(undefined, { maximumFractionDigits: 0 })}</Text>
            )}
