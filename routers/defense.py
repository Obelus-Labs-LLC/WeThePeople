...
def get_defense_companies(
    ...
    if q:
        ...
        query = query.filter(
            (TrackedDefenseCompany.display_name.ilike(pattern, escape="\\"))
            | (TrackedDefenseCompany.company_id.ilike(pattern, escape="\\"))
            | (TrackedDefenseCompany.ticker.ilike(pattern, escape="\\"))
        )
    if sector_type:
