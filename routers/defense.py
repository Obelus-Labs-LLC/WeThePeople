...
def get_defense_recent_activity(limit: int = Query(10, ge=1, le=30)):
...
def get_defense_company(company_id: str):
    db = SessionLocal()
    try:
...
def get_defense_company_contract_summary(company_id: str):
    db = SessionLocal()
    try:
        co = db.query(TrackedDefenseCompany).filter_by(company_id=company_id).first()
        if not co:
...
def get_defense_company_lobbying_summary(company_id: str):
    db = SessionLocal()
    try:
        co = db.query(TrackedDefenseCompany).filter_by(company_id=company_id).first()
        if not co:
...
def get_defense_company_stock(company_id: str):
    db = SessionLocal()
    try:
        co = db.query(TrackedDefenseCompany).filter_by(company_id=company_id).first()
        if not co:
...
@router.get("/compare")
def get_defense_comparison(ids: str = Query(..., description="Comma-separated company IDs")):
    db = SessionLocal()
    try:
