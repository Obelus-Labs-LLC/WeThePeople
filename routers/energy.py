...
def get_energy_dashboard_stats(db: Session = Depends(get_db)):
    ...
    by_sector = {}
    rows = db.query(TrackedEnergyCompany.sector_type, func.count()).filter(TrackedEnergyCompany.is_active == 1).group_by(TrackedEnergyCompany.sector_type).all()
...
def get_energy_recent_activity(limit: int = Query(10, ge=1, le=30), db: Session = Depends(get_db)):
...
def get_energy_companies(limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0), q: Optional[str] = Query(None), sector_type: Optional[str] = Query(None), db: Session = Depends(get_db)):
    ...
    results = []
    for co in companies:
        results.append({"company_id": co.company_id, "display_name": co.display_name, "ticker": co.ticker, "sector_type": co.sector_type, "headquarters": co.headquarters, "logo_url": co.logo_url, "emission_count": emission_counts.get(co.company_id, 0), "contract_count": contract_counts.get(co.company_id, 0), "filing_count": filing_counts.get(co.company_id, 0), "enforcement_count": enforcement_counts.get(co.company_id, 0)})
    return {"total": total, "limit": limit, "offset": offset, "companies": results}


@router.get("/companies/{company_id}")
def get_energy_company(company_id: str, db: Session = Depends(get_db)):
    logger.info("Energy company detail request: %s", company_id)
    ...
    latest_stock = None
    latest = db.query(StockFundamentals).filter_by(entity_type="energy_company", entity_id=company_id).order_by(desc(StockFundamentals.snapshot_date)).first()
    if latest:
    ...
    return {"company_id": co.company_id, "display_name": co.display_name, "ticker": co.ticker, "sector_type": co.sector_type, "headquarters": co.headquarters, "logo_url": co.logo_url, "sec_cik": co.sec_cik, "emission_count": emission_count, "contract_count": contract_count, "filing_count": filing_count, "enforcement_count": enforcement_count, "lobbying_count": lobbying_count, "total_contract_value": total_contract_value, "total_penalties": total_penalties, "latest_stock": latest_stock, "ai_profile_summary": co.ai_profile_summary, "sanctions_status": co.sanctions_status}


@router.get("/companies/{company_id}/filings")
def get_energy_company_filings(company_id: str, form_type: Optional[str] = Query(None), limit: int = Query(25, ge=1, le=100), offset: int = Query(0, ge=0), db: Session = Depends(get_db)):
    co = db.query(TrackedEnergyCompany).filter_by(company_id=company_id).first()
    if not co: raise HTTPException(status_code=404, detail="Energy company not found")
    query = db.query(SECEnergyFiling).filter_by(company_id=company_id)
    if form_type: query = query.filter(SECEnergyFiling.form_type == form_type)
    total = query.count()
...
def get_energy_company_emissions(company_id: str, reporting_year: Optional[int] = Query(None), limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0), db: Session = Depends(get_db)):
    co = db.query(TrackedEnergyCompany).filter_by(company_id=company_id).first()
    if not co: raise HTTPException(status_code=404, detail="Energy company not found")
    query = db.query(EnergyEmission).filter_by(company_id=company_id)
    if reporting_year: query = query.filter(EnergyEmission.reporting_year == reporting_year)
    total = query.count()
    ...
    return {"total": total, "total_co2e": total_co2e, "limit": limit, "offset": offset, "emissions": [{"id": e.id, "facility_name": e.facility_name, "facility_id_epa": getattr(e, 'facility_id_epa', None), "facility_city": getattr(e, 'facility_city', None), "facility_state": e.facility_state, "reporting_year": e.reporting_year, "total_emissions": e.total_emissions, "emission_type": e.emission_type, "industry_type": e.industry_type, "source_url": e.source_url} for e in records]}


@router.get("/companies/{company_id}/emissions/summary")
def get_energy_company_emissions_summary(company_id: str, db: Session = Depends(get_db)):
    co = db.query(TrackedEnergyCompany).filter_by(company_id=company_id).first()
    if not co: raise HTTPException(status_code=404, detail="Energy company not found")
    total_records = db.query(EnergyEmission).filter_by(company_id=company_id).count()
    total_co2e = db.query(func.sum(EnergyEmission.total_emissions)).filter_by(company_id=company_id).scalar() or 0
    by_year = {}
    rows = db.query(EnergyEmission.reporting_year, func.sum(EnergyEmission.total_emissions), func.count()).filter_by(company_id=company_id).group_by(EnergyEmission.reporting_year).order_by(EnergyEmission.reporting_year).all()
    for year, emissions, count in rows: by_year[str(year)] = {"total_emissions": emissions or 0, "facilities": count}
    by_state = {}
    rows = db.query(EnergyEmission.facility_state, func.sum(EnergyEmission.total_emissions), func.count()).filter_by(company_id=company_id).group_by(EnergyEmission.facility_state).order_by(func.sum(EnergyEmission.total_emissions).desc()).limit(10).all()
    for state, emissions, count in rows:
        if state: by_state[state] = {"total_emissions": emissions or 0, "facilities": count}
    year_keys = sorted(by_year.keys())
    yoy_change = None
    if len(year_keys) >= 2:
        curr = by_year[year_keys[-1]]["total_emissions"]
        prev = by_year[year_keys[-2]]["total_emissions"]
        if prev and prev > 0: yoy_change = round(((curr - prev) / prev) * 100, 1)
    climate_keywords = ["climate", "emission", "greenhouse", "carbon", "clean energy", "renewable", "epa", "environmental", "pollution", "methane"]
    ...
    climate_lobbying_count = db.query(func.count(EnergyLobbyingRecord.id)).filter(EnergyLobbyingRecord.company_id == company_id, or_(*climate_lobby_filters)).scalar() or 0
    climate_lobbying_spend = db.query(func.sum(EnergyLobbyingRecord.income)).filter(EnergyLobbyingRecord.company_id == company_id, or_(*climate_lobby_filters)).scalar() or 0
    return {"total_records": total_records, "total_co2e": total_co2e, "by_year": by_year, "by_state": by_state, "yoy_change_pct": yoy_change, "climate_lobbying_count": climate_lobbying_count, "climate_lobbying_spend": float(climate_lobbying_spend)}


@router.get("/companies/{company_id}/contracts")
def get_energy_company_contracts(company_id: str, limit: int = Query(25, ge=1, le=100), offset: int = Query(0, ge=0), db: Session = Depends(get_db)):
    co = db.query(TrackedEnergyCompany).filter_by(company_id=company_id).first()
    if not co: raise HTTPException(status_code=404, detail="Energy company not found")
...
def get_energy_company_contract_summary(company_id: str, db: Session = Depends(get_db)):
    co = db.query(TrackedEnergyCompany).filter_by(company_id=company_id).first()
    if not co: raise HTTPException(status_code=404, detail="Energy company not found")
    ...
    by_agency = {}
    rows = db.query(EnergyGovernmentContract.awarding_agency, func.count()).filter_by(company_id=company_id).group_by(EnergyGovernmentContract.awarding_agency).order_by(func.count().desc()).limit(10).all()
...
def get_energy_company_lobbying(company_id: str, filing_year: Optional[int] = Query(None), limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0), db: Session = Depends(get_db)):
    co = db.query(TrackedEnergyCompany).filter_by(company_id=company_id).first()
    if not co: raise HTTPException(status_code=404, detail="Energy company not found")
    query = db.query(EnergyLobbyingRecord).filter_by(company_id=company_id)
    if filing_year: query = query.filter(EnergyLobbyingRecord.filing_year == filing_year)
    total = query.count()
...
def get_energy_company_lobbying_summary(company_id: str, db: Session = Depends(get_db)):
    co = db.query(TrackedEnergyCompany).filter_by(company_id=company_id).first()
    if not co: raise HTTPException(status_code=404, detail="Energy company not found")
    ...
    by_year = {}
    rows = db.query(EnergyLobbyingRecord.filing_year, func.sum(EnergyLobbyingRecord.income), func.count()).filter_by(company_id=company_id).group_by(EnergyLobbyingRecord.filing_year).order_by(EnergyLobbyingRecord.filing_year).all()
    for year, income, count in rows: by_year[str(year)] = {"income": income or 0, "filings": count}
    top_firms = {}
    rows = db.query(EnergyLobbyingRecord.registrant_name, func.sum(EnergyLobbyingRecord.income), func.count()).filter_by(company_id=company_id).group_by(EnergyLobbyingRecord.registrant_name).order_by(func.sum(EnergyLobbyingRecord.income).desc()).limit(10).all()
...
def get_energy_company_enforcement(company_id: str, limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0), db: Session = Depends(get_db)):
    co = db.query(TrackedEnergyCompany).filter_by(company_id=company_id).first()
    if not co: raise HTTPException(status_code=404, detail="Energy company not found")
...
def get_energy_company_stock(company_id: str, db: Session = Depends(get_db)):
    co = db.query(TrackedEnergyCompany).filter_by(company_id=company_id).first()
    if not co: raise HTTPException(status_code=404, detail="Energy company not found")
    latest = db.query(StockFundamentals).filter_by(entity_type="energy_company", entity_id=company_id).order_by(desc(StockFundamentals.snapshot_date)).first()
...
@router.get("/compare")
def get_energy_comparison(ids: str = Query(..., description="Comma-separated company IDs"), db: Session = Depends(get_db)):
    ...
    for cid in company_ids:
        latest = db.query(StockFundamentals).filter_by(entity_type="energy_company", entity_id=cid).order_by(desc(StockFundamentals.snapshot_date)).first()
    ...
    for cid in company_ids:
        ...
        results.append({"company_id": co.company_id, "display_name": co.display_name, "ticker": co.ticker, "sector_type": co.sector_type, "emission_count": emission_counts.get(cid, 0), "total_emissions": float(emission_totals.get(cid, 0) or 0), "contract_count": contract_counts.get(cid, 0), "total_contract_value": float(contract_values.get(cid, 0) or 0), "lobbying_total": float(lobbying_totals.get(cid, 0) or 0), "enforcement_count": enforcement_counts.get(cid, 0), "total_penalties": float(penalty_totals.get(cid, 0) or 0), "market_cap": latest.market_cap if latest else None, "pe_ratio": latest.pe_ratio if latest else None, "profit_margin": latest.profit_margin if latest else None})
    return {"companies": results}


@router.get("/companies/{company_id}/donations")
def get_energy_company_donations(company_id: str, limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0), db: Session = Depends(get_db)):
...
def get_energy_company_trends(company_id: str, db: Session = Depends(get_db)):
    ...
    return {"years": years, "series": {"lobbying": [lobby_by_year.get(y, 0) for y in years], "contracts": [contracts_by_year.get(y, 0) for y in years], "enforcement": [enforcement_by_year.get(y, 0) for y in years], "emissions": [emissions_by_year.get(y, 0) for y in years]}}
