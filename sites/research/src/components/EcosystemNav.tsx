 ...
 * Token-driven version to match the unified redesign (Apr 2026).
 */
...
const ACCENT = 'var(--color-research)';
            ...
            {sites.map((site, i) => (
              <span key={site.key} className="flex items-center">
                {i > 0 && (
                  <span
                    aria-hidden
                    style={{ color: 'var(--color-text-3)', margin: '0 2px', opacity: 0.5 }}
                  >
                    |
                  </span>
                )}
                {site.key === active ? (
                  <span style={{ ...labelStyle, color: ACCENT }}>{site.label}</span>
                ) : (
                  <a
                    href={site.href}
                    style={{ ...labelStyle, color: 'var(--color-text-3)', transition: 'color 0.2s' }}
