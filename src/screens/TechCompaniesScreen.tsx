...
import type { TechCompany } from '../api/types';
...
type SectorFilter = 'all' | 'platform' | 'enterprise' | 'semiconductor' | 'automotive' | 'media';

const SECTOR_OPTIONS: { key: SectorFilter; label: string }[] = [
  { key: 'all', label: 'All' },
  { key: 'platform', label: 'Platform' },
  { key: 'enterprise', label: 'Enterprise' },
  { key: 'semiconductor', label: 'Chips' },
  { key: 'automotive', label: 'Automotive' },
  { key: 'media', label: 'Media' },
];

export default function TechCompaniesScreen() {
  ...
  const loadData = async () => {
    const res = await apiClient.getTechCompanies({ limit: 200 });
  ...
  const renderCompany = ({ item: c }: { item: TechCompany }) => (
    <TouchableOpacity
      style={styles.card}
