...
type Tab = 'overview' | 'contracts' | 'lobbying' | 'enforcement' | 'sec_filings' | 'donations';

export default function TransportationCompanyScreen() {
  const route = useRoute<any>();
  const navigation = useNavigation<any>();
  const companyId: string = route.params?.company_id;

  const [company, setCompany] = useState<TransportationCompanyDetail | null>(null);
  ...
  const loadCompany = useCallback(async () => {
    try {
      const detail = await apiClient.getTransportationCompanyDetail(companyId);
      setCompany(detail);
  ...
  useEffect(() => {
    ...
    if (tab === 'contracts' && !contractsLoaded && !contractsLoading) {
      ...
      Promise.all([
        apiClient.getTransportationCompanyContracts(companyId, { limit: 25 }),
        apiClient.getTransportationCompanyContractSummary(companyId),
      ])
        .then(([cRes, sRes]) => {
    ...
    if (tab === 'lobbying' && !lobbyLoaded && !lobbyLoading) {
      ...
      Promise.all([
        apiClient.getTransportationCompanyLobbying(companyId, { limit: 20 }),
        apiClient.getTransportationCompanyLobbySummary(companyId),
      ])
        .then(([lRes, sRes]) => {
    ...
    if (tab === 'enforcement' && !enforcementLoaded && !enforcementLoading) {
      ...
      apiClient.getTransportationCompanyEnforcement(companyId, { limit: 50 })
        .then(r => {
    ...
    if (tab === 'donations' && donations === null && !donationsLoading) {
      ...
      apiClient.getTransportationCompanyDonations(companyId, { limit: 50 })
        .then((res) => setDonations(res.donations || res || []))
        .catch(() => setDonations([]))
        .finally(() => setDonationsLoading(false));
    }
    if (tab === 'sec_filings' && !secFilingsLoaded && !secFilingsLoading) {
      setSecFilingsLoading(true);
      apiClient.getTransportationCompanyFilings(companyId, { limit: 25 })
        .then((res) => setSecFilings(res.filings || []))
        .catch(() => {})
        .finally(() => { setSecFilingsLoading(false); setSecFilingsLoaded(true); });
    }
  }, [tab, company, companyId, contractsLoaded, lobbyLoaded, enforcementLoaded, contractsLoading, lobbyLoading, enforcementLoading, donations, donationsLoading, secFilingsLoaded, secFilingsLoading]);
  ...
  const renderEnforcement = () => {
    ...
    if (enforcement.length === 0) return <EmptyState title="No Enforcement Actions" message="No FAA/NHTSA/DOT actions found" />;
    return (
      <>
        {totalPenalties > 0 && (
          <View style={styles.summaryRow}>
  ...
  const renderSecFilings = () => {
    if (secFilingsLoading) return <LoadingSpinner message="Loading SEC filings..." />;
    if (secFilings.length === 0) return <EmptyState title="No SEC Filings" message="No filings found for this company" />;
    return (
      <FlatList
        data={secFilings}
        scrollEnabled={false}
        keyExtractor={(item) => String(item.id || item.accession_number)}
        renderItem={({ item: f }) => (
          <TouchableOpacity
            style={styles.listCard}
            onPress={() => f.filing_url ? Linking.openURL(f.filing_url) : null}
            accessibilityRole="link"
            accessibilityLabel={`Filing: ${f.form_type || 'SEC Filing'}`}
          >
            <View style={{ flex: 1 }}>
              <Text style={styles.listTitle} numberOfLines={1}>{f.form_type || 'Filing'} — {f.filing_date || ''}</Text>
              {f.description && <Text style={styles.listMeta} numberOfLines={2}>{f.description}</Text>}
            </View>
            {f.filing_url && <Ionicons name="open-outline" size={16} color={sectorColor} />}
          </TouchableOpacity>
        )}
      />
    );
    ...
    >
      ...
      <View style={styles.header}>
        <View style={[styles.headerIcon, { backgroundColor: sectorColor + '15' }]}>
          <Ionicons name="airplane" size={28} color={sectorColor} />
        </View>
      ...
      <View style={styles.tabContent}>
        ...
        {tab === 'sec_filings' && renderSecFilings()}
